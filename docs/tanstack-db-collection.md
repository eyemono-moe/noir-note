...
interface MyCollectionConfig<TItem extends object> extends Omit<
  ...
  return {
    onInsert: async ({ transaction }) => {
      ...
      const serialized = transaction.mutations.map((m) => ({
        ...
        created_at:
          m.modified.created_at instanceof Date
...
export function myCollectionOptions<TItem extends object>(config: MyCollectionConfig<TItem>) {
  return {
    // ... other options
  ...
  return {
    ...
    onInsert: async ({ transaction }) => {
      ...
      for (const chunk of chunks) {
        const ids = await config.recordApi.createBulk(chunk.map((m) => serialize(m.modified)));
        await awaitIds(ids);
      }
    ...
    onUpdate: async ({ transaction }) => {
      ...
      for (const chunk of chunks) {
        await Promise.all(chunk.map((m) => config.recordApi.update(m.key, serialize(m.changes))));
      }
