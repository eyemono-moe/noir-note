  ...
  return (
    ...
    >
      <div class="flex flex-col gap-0.5 p-1">
        <Show
          when={refs.latest && refs.latest.length > 0}
          fallback={
            <div class="text-text-secondary px-4 py-8 text-center text-sm">No backlinks</div>
          }
        >
          <For each={refs.latest ?? []}>
            {(path) => (
              <HoverCard.Trigger
                value={path}
                asChild={(hoverProps) => (
                  <button
                    ...
                    {...hoverProps()}
                  >
                    <span class="i-material-symbols:link-rounded size-4 shrink-0" />
                    <span class="flex-1 truncate">{getMemoDisplayName(path, props.allMemos)}</span>
                  </button>
                )}
              />
            )}
